...
const CreateCampaign = () => {
  ...
  const validateForm = () => {
    ...
    const validRewards = rewards.filter(r => 
      r.title.trim() && r.description.trim() && parseFloat(r.amount) > 0
    );
          ...
          <div className="space-y-6">
              ...
              <div className="space-y-4">
                {rewards.map((reward, index) => (
                  <Card key={reward.id} className="p-4 bg-muted/30">
                    <div className="flex items-center justify-between mb-4">
                      <Badge variant="secondary">Recompensa {index + 1}</Badge>
                      {rewards.length > 1 && (
                        <Button
                          variant="ghost"
                          size="icon"
            ...
            <Card className="p-6 bg-gradient-hero border-primary/20">
              ...
              <ul className="space-y-2 text-sm text-muted-foreground">
                ...
                <li className="flex items-start gap-2">
                  <CheckCircle2 className="w-4 h-4 text-secondary flex-shrink-0 mt-0.5" />
                  <span>Se atingir a meta, você recebe os fundos em 24h após o término</span>
                </li>
                <li className="flex items-start gap-2">
                  <AlertCircle className="w-4 h-4 text-yellow-500 flex-shrink-0 mt-0.5" />
                  <span>Se não atingir, há estorno automático para os apoiadores</span>
                </li>
                <li className="flex items-start gap-2">
                  <CheckCircle2 className="w-4 h-4 text-secondary flex-shrink-0 mt-0.5" />
                  <span>Apoiadores podem pedir reembolso em até 24h após o apoio</span>
                </li>
                <li className="flex items-start gap-2">
                  <CheckCircle2 className="w-4 h-4 text-secondary flex-shrink-0 mt-0.5" />
                  <span>Saque liberado 1 dia após encerramento bem-sucedido</span>
                </li>
              </ul>
            </Card>
            ...
            <Card className="p-6 bg-card/50 backdrop-blur-sm">
              ...
              <ul className="space-y-2 text-sm">
                ...
                <li className="flex items-center gap-2">
                  {rewards.some(r => r.title && r.description && parseFloat(r.amount) > 0) ? (
                    <CheckCircle2 className="w-4 h-4 text-secondary" />
                  ) : (
                    <AlertCircle className="w-4 h-4 text-yellow-500" />
                  )}
